...
#' @usage assign_articles(articles, team_df, seed = 1)
#' @param df data frame of articles, should have at least title<str> and abstract<str> columns
#' @param team a data.frame of team members with name<chr>, and one or more effort <dbl> columns
...
#' assign_articles(merged_articles, team_df, "screening_effort", reviewer = 2, seed = 1)

library(dplyr)

assign_articles <- function(articles, team_df, effort_col, reviewers = 2, seed = 1){
    stopifnot(tibble::has_name(articles, c("title", "abstract")),
    ...
    articles %>%
        ...
        mutate(reviewer = sample(team_df$name, size = reviewers,
                                 prob = pull(team_df, !!effort_col)) %>%
                          paste(., collapse = ","),
